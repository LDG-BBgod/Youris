...
for (let i = 0; i < 7; i++){
    ...
    list[i].addEventListener('click', () => {
        ...
        for (let j = 0; j < 12; j++){
            document.getElementById(`time-list${j}`).classList.remove('select', 'imposible')
    ...
    $.ajax({
        ...
        success: function(requestData) {
            ...
            for (let i = 0; i < requestData.length; i++){
                for (let j = 0; j < 12; j++){
                    if(reserveTime[j].innerHTML.substr(0,11) == requestData[i].fields.consultingTime){
...
//////////  연락처  //////////
let callType = [document.getElementsByClassName(`selectType-list0`)[0], document.getElementsByClassName(`selectType-list1`)[0]]
...
function submitFunction() {
    ...
    for (let i = 0; i < 7; i++){
        if (document.getElementsByClassName(`date${i}`)[0].classList.item(2) == 'select') {
    ...
    for (let i = 0; i < 12; i++){
        if (document.getElementById(`time-list${i}`).classList.item(0) == 'select') {
            dataArr[1] = document.getElementById(`time-list${i}`).innerHTML.substr(0,11)
            timeError = false
            break
        }
        else {
    ...
    for (let i = 0; i < 2; i++){
        if (document.getElementsByClassName(`selectType-list${i}`)[0].classList.item(2) == 'select') {
            dataArr[2] = document.getElementsByClassName(`selectType-list${i}`)[0].innerHTML
    ...
    if (dateError) {
        document.getElementsByClassName('error')[0].innerHTML = '날짜를 선택해주세요'
    }
    else if (timeError) {
        document.getElementsByClassName('error')[0].innerHTML = '시간을 선택해주세요'
    }
    else if (selectTypeError) {
        document.getElementsByClassName('error')[0].innerHTML = '상담 수단을 선택해주세요'
    }
    else if (agreeError) {
        document.getElementsByClassName('error')[0].innerHTML = '개인정보 처리방침 동의를 체크해주세요'
    }
    else if (inputPhoneError) {
        document.getElementsByClassName('error')[0].innerHTML = '전화번호나 카톡 아이디를 입력해주세요'
    }
    else {
        let overlap = false
        $.ajax({
        ...
        if (overlap){
            alert('다른 고객님께서 해당 시간을 예약하셨습니다. 다시 선택 부탁드립니다.')
